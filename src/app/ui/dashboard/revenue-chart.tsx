    ...
    <div className="w-full md:col-span-4">
        ...
        <div className="p-6 bg-gray-100 rounded-lg">
            <div className="p-2 flex flex-row bg-white gap-4">
                <div className="hidden sm:flex flex-col justify-between text-sm" style={{height: `${chartHeight}px`}}>
                    {
                        yLabelsAxis.map((label) => (
                            <p key={label} className="text-sm text-gray-400">{label}</p>
                        ))
                    }
                </div>
                <div className="grow grid grid-cols-12 items-end gap-2 md:gap-4">
                    {
                        revenue.map((rev) => (
                            <div key={rev.month} className="flex flex-col items-center gap-3">
                                <div
                                    className="bg-green-400 rounded-lg w-full" 
                                    style={{height:`${(chartHeight / topLabel) * rev.revenue}px`}}
                                />
                                <p className="-rotate-90 sm:rotate-0 text-sm text-gray-600">{rev.month}</p>
                            </div>
                        ))
                    }
                </div>
